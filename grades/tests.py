...
class GradeTest(TestCase):
    ...
    def testCreate(self):
        self.client.force_authenticate(user=self.student1)
        data = {'submission': 'http://testserver' + reverse('submission-list') + str(self.submission['id']) + '/',
                'final_grade': 'A',
                'notes': 'passed'}
        response = self.client.post('/grades/', data, format='json')
    ...
    def testList(self):
        ...
        response = self.client.get('/grades/', format='json')
